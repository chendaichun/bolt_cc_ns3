...
// $./waf --run "scratch/bolt-link-utilization-experiment --simIdx=2
//  --traceQueues --duration=0.005 --rttSmoothingAlpha=0.5
//  --topoScalingPerHop=250 --baseDelay=3500 --aiFactor=1.0
//  --mdFactor=0.8 --maxMd=0.5"
...
static void
calTpt (Ptr<OutputStreamWrapper> stream, uint32_t oldval, uint32_t newval)
{
  NS_LOG_INFO (Simulator::Now ().GetNanoSeconds ()
               << " Queue Disc size from " << oldval << " to " << newval);
...
static void
calPru (Ptr<OutputStreamWrapper> stream,uint16_t oldval, uint16_t newval)
{
  NS_LOG_INFO (Simulator::Now ().GetNanoSeconds ()
               << " Queue Disc size from " << oldval << " to " << newval);
  //NS_LOG_WARN ("wenzhu");
...
NetDeviceContainer senderToSwitchDevices[2]; //nFlows
NetDeviceContainer receiverToSwitchDevices[2]; //nFlows
...
Ipv4InterfaceContainer receiverToSwitchIfs[2]; //nFlows
Ipv4InterfaceContainer senderToSwitchIfs[2]; //nFlows
...
main (int argc, char *argv[])
{
  ...
  std::string outputDir = "outputs/bolt-fairness/";
  ...
  std::string tracesFileName ("outputs/bolt-fairness/");
  if (debugMode)
  ...
  hostLinks2.SetDeviceAttribute ("DataRate", StringValue ("102Gbps"));
  hostLinks2.SetChannelAttribute ("Delay", StringValue ("10us"));
  hostLinks2.SetQueue ("ns3::DropTailQueue", "MaxSize", StringValue ("1p"));
